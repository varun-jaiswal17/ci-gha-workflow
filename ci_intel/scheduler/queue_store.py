...
class QueueStore:
    ...
    def try_claim(self, run_id, max_concurrent, stale_ttl, now):
        ...
        def mutate(queue):
            ...
            claimed = False
            if len(running) < max_concurrent and waiting and waiting[0]["run_id"] == run_id:
    ...
    def force_claim(self, run_id, now):
        """Fail-open: mark this run running regardless of position (timeout escape)."""
        def mutate(queue):
            entries = queue.setdefault("entries", [])
            found = False
            for e in entries:
            ...
            if not found:
                entries.append({
                    ...
                    "score": 0, "state": "running", "heartbeat": now,
                })
            return queue, None

        self._transaction(mutate, f"gate: force-claim run {run_id}")

    def release(self, run_id):
        """Remove this run's entry, freeing its slot for the next waiter."""
        def mutate(queue):
            entries = queue.setdefault("entries", [])
            queue["entries"] = [e for e in entries if e["run_id"] != run_id]
            return queue, None

        self._transaction(mutate, f"gate: release run {run_id}")
